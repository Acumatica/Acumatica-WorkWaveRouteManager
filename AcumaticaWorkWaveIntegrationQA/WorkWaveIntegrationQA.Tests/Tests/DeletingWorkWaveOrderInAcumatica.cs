...
{
    public class DeletingWorkWaveOrderInAcumatica: TestBase
    {
        ...
        {
            using (TestExecution.CreateTestCaseGroup("Deleting the WorkWave order in Acumatica"))
            {
                ...
                using (TestExecution.CreateTestStepGroup("Step 8 - Delete the WorkWave order. Validate a warning message"))
                {
                    ...
                    DeleteWorkWaveOrder(_shipmentNumber);
                }
                using (TestExecution.CreateTestStepGroup("Step 9 - Validate the removed WorkWave order by Api"))
                {
                    //_orderId = "ab62cc45-2249-470b-a5c8-b5a0ee2cb0ef";
                    AssertRemovedWorkWaveOrder(_orderId);
                }
                using (TestExecution.CreateTestStepGroup("Step 10 - Create the second WorkWave order for current shipment"))
                {
                    //_shipmentNumber = "003668";
                    _requestId = CreateWorkWaveOrder(_shipmentNumber);
                }
                using (TestExecution.CreateTestStepGroup("Step 11 - Validate the just created WorkWave order"))
                {
                    //_shipmentNumber = "003669";
                    var expectedWorkWaveOrderData = new Dictionary<string, string>()
                    {
                ...
                using (TestExecution.CreateTestStepGroup("Step 12 - Get the content from Webhook by Request Id"))
                {
                    //_requestId = "866e469f-25cf-44f6-903c-e525193823a8";
                    _content = GetWebhookContentByGuid(_requestId);
                }
                using (TestExecution.CreateTestStepGroup("Step 13 - Send the POST request to update the WorkWave order in the Acumatica"))
                {
                    UpdateWorkWaveOrder(_content);
                }
                using (TestExecution.CreateTestStepGroup("Step 14 - Get the WorkWave order Ids"))
                {
                    var orderIdList = GetWorkWaveOrderIdList(_shipmentNumber);
                    _orderId = orderIdList.First();
                }
                using (TestExecution.CreateTestStepGroup("Step 15 - Validate the updated WorkWave order"))
                {
                    //_shipmentNumber = "003669";
                    var expectedWorkWaveOrderData = new Dictionary<string, string>()
                    {
                ...
                using (TestExecution.CreateTestStepGroup("Step 16 - Set the Vehicle by WorkWave Api"))
                {
                    _vehicleId = SetVehicleByWorkWaveApi(_orderId);
                }
                using (TestExecution.CreateTestStepGroup("Step 17 - Build and approve routes by WorkWave Api"))
                {
                    BuildAndApproveRoutes();
                }
                using (TestExecution.CreateTestStepGroup("Step 18 - Validate the updated WorkWave order"))
                {
                ...
                using (TestExecution.CreateTestStepGroup("Step 19 - Validate disabled action to delete the WorkWave order"))
                {
                    AssertDisabledActionDeleteWorkWaveOrder(_shipmentNumber);
                }
                using (TestExecution.CreateTestStepGroup("Step 20 - Complete PickUp and Delivery orders by Api"))
                {
                    var dateString = DateTime.Now.ToString("yyyyMMdd");
                    HandlePickUpAndDelivery(_orderId, _vehicleId, dateString);
                }
                using (TestExecution.CreateTestStepGroup("Step 21 - Validate the updated WorkWave order"))
                {
